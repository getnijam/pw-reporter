...
- **Expected failures**: the reported status is the one **Playwright** judges, never the raw `result.status` (`attemptStatus` in `reporter.ts`). Playwright's rule: an attempt is expected when `result.status === test.expectedStatus`, so a `test.fail()` test that fails is sent as `passed` (its intentional error is dropped) and one that passes is sent as `failed` with Playwright's own `Expected to fail, but passed.` message; `skipped`/`interrupted` keep their status. Run stats already roll up `test.outcome()`, so both levels agree with `playwright test`'s summary. Do NOT go back to mapping `result.status` straight through, that reported every `test.fail()` test as a red failure.
